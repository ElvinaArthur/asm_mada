'use client';
// components/auth/ProtectedRoute.tsx - NEXT.JS APP ROUTER
import React, { useEffect } from "react";
import { useRouter } from "next/navigation";
import { useAuth } from "../../hooks/AuthContext";

interface ProtectedRouteProps {
  children: React.ReactNode;
  requireVerification?: boolean;
  adminOnly?: boolean;
}

const ProtectedRoute = ({
  children,
  requireVerification = false,
  adminOnly = false,
}: ProtectedRouteProps) => {
  const { user, loading } = useAuth() as { user: { role?: string; isVerified?: boolean | number } | null; loading: boolean };
  const router = useRouter();

  useEffect(() => {
    if (!loading && !user) {
      router.push("/auth?mode=login");
    }
  }, [loading, user, router]);

  // Loading state
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <div className="w-16 h-16 border-4 border-green-600 border-t-transparent rounded-full animate-spin mx-auto mb-4"></div>
          <p className="text-gray-600">Chargement...</p>
        </div>
      </div>
    );
  }

  // Pas connecté → return null (redirect handled by useEffect)
  if (!user) {
    return null;
  }

  // Admin only
  if (adminOnly && user.role !== "admin") {
    return (
      <div className="min-h-screen flex items-center justify-center p-4">
        <div className="max-w-md w-full text-center bg-white rounded-2xl shadow-xl p-8">
          <div className="w-16 h-16 bg-red-100 rounded-full flex items-center justify-center mx-auto mb-6">
            <span className="text-3xl">🚫</span>
          </div>
          <h2 className="text-2xl font-bold text-gray-900 mb-4">
            Accès restreint
          </h2>
          <p className="text-gray-600 mb-6">
            Cette section est réservée aux administrateurs.
          </p>
          <a
            href="/dashboard"
            className="inline-block px-6 py-3 bg-green-600 text-white font-semibold rounded-lg hover:bg-green-700 transition"
          >
            Retour au tableau de bord
          </a>
        </div>
      </div>
    );
  }

  // Vérification requise
  if (requireVerification && !user.isVerified) {
    return (
      <div className="min-h-screen flex items-center justify-center p-4">
        <div className="max-w-md w-full text-center bg-white rounded-2xl shadow-xl p-8">
          <div className="w-16 h-16 bg-yellow-100 rounded-full flex items-center justify-center mx-auto mb-6">
            <span className="text-3xl">⏳</span>
          </div>
          <h2 className="text-2xl font-bold text-gray-900 mb-4">
            Vérification en cours
          </h2>
          <p className="text-gray-600 mb-6">
            Votre compte est en cours de vérification par l&apos;administrateur.
          </p>
          <a
            href="/dashboard"
            className="inline-block px-6 py-3 bg-green-600 text-white font-semibold rounded-lg hover:bg-green-700 transition"
          >
            Retour au tableau de bord
          </a>
        </div>
      </div>
    );
  }

  return <>{children}</>;
};

export default ProtectedRoute;
