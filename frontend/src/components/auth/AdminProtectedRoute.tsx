'use client';
// components/auth/AdminProtectedRoute.tsx - NEXT.JS APP ROUTER
...
const AdminProtectedRoute = ({ children }: AdminProtectedRouteProps) => {
  const { user, loading, isAdmin } = useAuth() as { user: { role?: string; email?: string } | null; loading: boolean; isAdmin: boolean };
  const router = useRouter();
  const pathname = usePathname();

  useEffect(() => {
    if (!loading && !user) {
      router.push(`/auth?mode=login&redirect=${encodeURIComponent(pathname || '/admin')}`);
    }
